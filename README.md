...
| ---- | ---- | ---- |
|local|関数のローカル変数を格納する| 関数に入ると動的に割り当てられる|
| argument | 関数の引数を格納する |　関数に入ると動的に割り当てられる |
| static | スタティック変数 |.vmファイルの全ての関数で共有される|
| constant| 定数地を持つ疑似セグメント|プログラムの全ての関数から見える|
| this that| 異なるヒープ領域に対応するように作られている。プログラミングの様々なニーズに用いられる|どのような関数からでもこれらのセグメントを使用できる(Objectと配列に使用されることが多い)|
| pointer| this thatセグメントのベースアドレスを持つ二つの要素からなるセグメント|this,thatセグメントのポインターを格納することが出来る|
| temp| 固定された8つの要素からなるセグメント。一時的な変数を格納するために用いられる|プログラムの全ての関数から利用できる変数領域|
